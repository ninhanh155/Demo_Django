...
def diendan_cacchude(req,id):
    dd = get_object_or_404(Diendan, id=id)
    cd = dd.cac_chude
    return render(req, 'cac_chu_de.html', {'dd':dd})
...
def tao_chude_moi(req,id):
    dd = get_object_or_404(Diendan, id=id)
    # user = User.objects.first()   #--> lấy người dùng hiện đang đăng nhập
...
def xem_chude_thaoluan(req, ddpk, cdpk):
    cd = get_object_or_404(Chude, dien_dan__pk = ddpk ,pk = cdpk )
    cd.luot_xem += 1
    cd.save()
    return render(req, 'chude_thaoluan.html', {'cd': cd})
